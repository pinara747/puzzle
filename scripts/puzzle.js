	...
	for (i=0;i<height;i++){
		for (j=0;j<wid;j++){
			if(position[nameImage]>=0 && position[nameImage]<numTiles){
				document.images["lion"+numPositionBoard].src=pics[4+((1+position[nameImage])%19)].src;
			}else{
				document.images["lion"+numPositionBoard].src=pics[22].src;
			}
			nameImage++;
			numPositionBoard++;
		}
...
for(var i=0;i<maxheight;i++){
	...
	for(var j=0;j<maxwidth;j++){
		document.writeln("  <td width=20><a href='javascript:clicked("+i+","+j+");focus();'>"+
			"<img src='"+pics[22].src+"' height=158 width=158 border=1 style='border-color:#ffffff;' name='lion"+nameImage+"'>"+
			"<\/a><\/td>");
		nameImage++;
	}
	document.writeln("<\/tr>");
...
function show(y,x){
	...
	if(position[c]>=0 && position[c]<numTiles){
		document.images["lion"+d].src=pics[4+((1+position[c])%19)].src;
	}else{
		document.images["lion"+d].src=pics[21].src;
	}
}
